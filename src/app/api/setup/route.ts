...
const SETUP_SQL: string[] = [
  ...
  `CREATE TABLE IF NOT EXISTS audit_logs (id INT AUTO_INCREMENT PRIMARY KEY, user_id INT NULL, action ENUM('login','logout','generate_voucher','delete_voucher','delete_client','toggle_router','record_payment','change_password','create_client','update_client','failed_login','password_reset_request','password_reset') NOT NULL, details TEXT, ip_address VARCHAR(45), user_agent TEXT, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users(id)) ENGINE=InnoDB;`,
  `CREATE TABLE IF NOT EXISTS portal_orders (id INT AUTO_INCREMENT PRIMARY KEY, client_id INT NOT NULL, profile_id INT NOT NULL, phone VARCHAR(20) NOT NULL, amount DECIMAL(10,2) NOT NULL, status ENUM('pending','processing','paid_pending_fulfillment','paid','failed','cancelled') NOT NULL DEFAULT 'pending', checkout_request_id VARCHAR(100) UNIQUE, mpesa_receipt VARCHAR(50) UNIQUE, voucher_id INT NULL, voucher_code VARCHAR(20), created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, completed_at TIMESTAMP NULL, FOREIGN KEY (client_id) REFERENCES clients(id), FOREIGN KEY (profile_id) REFERENCES voucher_profiles(id), FOREIGN KEY (voucher_id) REFERENCES vouchers(id)) ENGINE=InnoDB;`,
  ...
  `CREATE TABLE IF NOT EXISTS router_sync_events (id INT AUTO_INCREMENT PRIMARY KEY, client_id INT NOT NULL, event_id VARCHAR(100) NOT NULL UNIQUE, event_type VARCHAR(50) NOT NULL, payload TEXT NOT NULL, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, received_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (client_id) REFERENCES clients(id), INDEX router_sync_events_client (client_id, created_at)) ENGINE=InnoDB;`,
...
export async function GET(request: NextRequest) {
  ...
  if (!setupSecret || suppliedSecret !== setupSecret) {
    return NextResponse.json({ success: false, error: "Setup imefungwa" }, { status: 404 });
  }
  ...
  try {
    ...
    for (const statement of [
      "ALTER TABLE portal_orders MODIFY status ENUM('pending','processing','paid_pending_fulfillment','paid','failed','cancelled') NOT NULL DEFAULT 'pending'",
      "ALTER TABLE portal_orders ADD UNIQUE INDEX portal_orders_checkout_request_id (checkout_request_id)",
      "ALTER TABLE portal_orders ADD UNIQUE INDEX portal_orders_mpesa_receipt (mpesa_receipt)",
    ]) {
      try {
        await pool.query(statement);
      } catch (err) {
    ...
    if (userCount === 0) {
      ...
      const vendorPassword = process.env.INITIAL_VENDOR_PASSWORD;
      if (!adminPassword || !vendorPassword) {
        throw new Error("INITIAL_ADMIN_PASSWORD na INITIAL_VENDOR_PASSWORD zinahitajika kwa setup ya kwanza");
      }
      ...
      seeded = true;
      credentials = { adminUsername: "Rajabu", vendorUsername: "juma_wifi" };
